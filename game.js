...
function resetGame() { const scale=getResponsiveScale(); score=0; health=100; wave=1; waveKills=0; spawnTimer=0; waveDelay=0; powerUpTimer=8; boss=null; bullets=[]; enemyBullets=[]; enemies=[]; powerUps=[]; particles=[]; player={x:width/2,y:height-105*scale,w:42*scale,h:58*scale,cooldown:0,shield:0,overload:0,overheated:0,timeSinceShot:1}; clouds=Array.from({length:8},(_,i)=>({x:(i*173)%width,y:30+(i*91)%(height*.72),s:(.5+(i%3)*.22)*scale,speed:5+i%4})); updateHud(); }
...
function getCombatBounds() { const scale=getResponsiveScale(); const margin = Math.max(combatLane.edge * scale, width * (width < 650 ? .16 : combatLane.margin)); return { min:margin, max:width-margin }; }
...
function spawnBoss() { if(enemies.length>0) return; const scale=getResponsiveScale(); const bounds = getCombatBounds(); boss={x:(bounds.min+bounds.max)/2,y:-90*scale,w:150*scale,h:90*scale,speed:42,hp:30,maxHp:30,phase:0,ammo:3,maxAmmo:3,shotTimer:3,reloadTimer:0}; enemies=[]; playTone(90,.5,'sawtooth',.08,45); }
...
function draw() {const scale=getResponsiveScale();drawBackground();powerUps.forEach(powerUp=>{ctx.save();ctx.translate(powerUp.x,powerUp.y);ctx.rotate(powerUp.rotation);ctx.fillStyle='#75c9d8';ctx.shadowBlur=14*scale;ctx.shadowColor='#75c9d8';ctx.fillRect(-10*scale,-10*scale,20*scale,20*scale);ctx.fillStyle='#173c4c';ctx.fillRect(-3*scale,-8*scale,6*scale,16*scale);ctx.fillRect(-8*scale,-3*scale,16*scale,6*scale);ctx.restore();});enemyBullets.forEach(b=>{ctx.fillStyle='#d85e3f';ctx.shadowBlur=12*scale;ctx.shadowColor='#d85e3f';ctx.fillRect(b.x-4*scale,b.y-8*scale,8*scale,16*scale);ctx.shadowBlur=0;});bullets.forEach(b=>{ctx.fillStyle='#ffe4a5';ctx.shadowBlur=12*scale;ctx.shadowColor='#ffba46';ctx.fillRect(b.x-2*scale,b.y-12*scale,4*scale,18*scale);ctx.shadowBlur=0;});enemies.forEach(e=>drawPlane(e.x,e.y,(e.type==='ace'?1.25:1)*scale,e.type==='ace'?'#5c666b':'#b5c2c5',e.type==='ace'?'su57':'mig35'));if(boss){if(boss.reloadTimer>0){ctx.strokeStyle='#75c9d8';ctx.lineWidth=4*scale;ctx.shadowBlur=18*scale;ctx.shadowColor='#75c9d8';ctx.beginPath();ctx.arc(boss.x,boss.y,70*scale,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;}drawPlane(boss.x,boss.y,1.5*scale,'#5c666b','b2');ctx.fillStyle='rgba(16,39,49,.75)';ctx.fillRect(boss.x-55*scale,boss.y-75*scale,110*scale,7*scale);ctx.fillStyle='#d85e3f';ctx.fillRect(boss.x-55*scale,boss.y-75*scale,110*scale*(boss.hp/boss.maxHp),7*scale);}if(player){if(player.shield>0){ctx.strokeStyle='#75c9d8';ctx.lineWidth=3*scale;ctx.shadowBlur=16*scale;ctx.shadowColor='#75c9d8';ctx.beginPath();ctx.arc(player.x,player.y,43*scale,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;}drawPlane(player.x,player.y,1.35*scale,'#3e4b59','player');drawPlayerBars();}particles.forEach(p=>{ctx.globalAlpha=Math.max(0,p.life/.7);ctx.fillStyle=p.color;ctx.fillRect(p.x,p.y,p.size*scale,p.size*scale);});ctx.globalAlpha=1;}
