...
function resetGame() { const scale=getResponsiveScale(); score=0; health=100; wave=1; waveKills=0; spawnTimer=0; waveDelay=0; powerUpTimer=8; boss=null; bullets=[]; enemyBullets=[]; lasers=[]; laserWarnings=[]; fallingThreats=[]; enemies=[]; powerUps=[]; particles=[]; player={x:width/2,y:height-105*scale,w:42*scale,h:58*scale,cooldown:0,shield:0,overload:0,overheated:0,timeSinceShot:1}; clouds=Array.from({length:8},(_,i)=>({x:(i*173)%width,y:30+(i*91)%(height*.72),s:(.5+(i%3)*.22)*scale,speed:5+i%4})); updateHud(); }
...
function getCombatBounds() { const scale=getResponsiveScale(); const margin = Math.max(combatLane.edge * scale, width * (width < 650 ? .22 : combatLane.margin)); return { min:margin, max:width-margin }; }
...
function spawnEnemy() { const scale=getResponsiveScale(); const type = wave > 1 && Math.random() > .78 ? 'ace' : 'fighter'; const bounds = getCombatBounds(); enemies.push({id:enemyId++,x:bounds.min+Math.random()*(bounds.max-bounds.min),y:-55*scale,w:(type==='ace'?50:38)*scale,h:(type==='ace'?66:53)*scale,speed:55+wave*8+Math.random()*35,type,hp:type==='ace'?2:1,phase:Math.random()*7}); }
...
function spawnBoss() { if(enemies.length>0) return; const scale=getResponsiveScale(); const bounds = getCombatBounds(); boss={x:(bounds.min+bounds.max)/2,y:-90*scale,w:150*scale,h:90*scale,speed:42,hp:30,maxHp:30,phase:0,attack:0,attackTimer:2,laserLaunchTimer:Infinity,transitionBatch:0,state:'normal',enraged:false}; enemies=[]; playTone(90,.5,'sawtooth',.08,45); }
function spawnBossTransitionWave() { const bounds=getCombatBounds(); for(let index=0;index<8;index++){enemies.push({id:enemyId++,x:bounds.min+(bounds.max-bounds.min)*(index+1)/9,y:-55-(index%3)*70,w:50,h:66,speed:70+Math.random()*25,type:'ace',hp:2,phase:Math.random()*7});} }
...
function getLaserPositions() { const bounds=getCombatBounds(), count=boss?.enraged?4:3, spacing=(bounds.max-bounds.min)/(count+1); return Array.from({length:count},(_,index)=>bounds.min+spacing*(index+1)); }
function bossBasicAttack() { const speed=300*(boss.enraged?bossFuriousSpeed:1), spread=15*Math.PI/180, origin={x:boss.x,y:boss.y+55}, projectileCount=boss.enraged?7:5; for(let index=-(projectileCount-1)/2;index<=(projectileCount-1)/2;index++){const angle=spread*index/2;enemyBullets.push({type:'basic',x:origin.x,y:origin.y,vx:Math.sin(angle)*speed,vy:Math.cos(angle)*speed});} playTone(120,.12,'square',.05,70); }
function bossLaserWarning() { laserWarnings=getLaserPositions().map(x=>({x,time:boss.enraged?1.5/bossFuriousSpeed:1.5})); playTone(260,.35,'sine',.04,520); }
function bossLaserAttack() { lasers=getLaserPositions().map(x=>({x,time:boss.enraged?.9/bossFuriousSpeed:.9})); laserWarnings=[]; playTone(90,.28,'sawtooth',.06,45); }
function bossBallAttack() { const bounds=getCombatBounds(), speedMultiplier=boss.enraged?bossFuriousSpeed:1; fallingThreats=Array.from({length:8},(_,index)=>({x:bounds.min+(bounds.max-bounds.min)*(index+1)/9,y:-60-index*105,speed:(145+index%3*18)*speedMultiplier,phase:index*.9,wobble:18})); playTone(170,.2,'square',.05,70); }
function bossFuriousCombo() { bossBasicAttack(); bossLaserWarning(); bossBallAttack(); boss.laserLaunchTimer=1.5/bossFuriousSpeed; playTone(100,.18,'sawtooth',.04,180); }
function getWaveTarget() { return wave >= 4 ? wave + 2 : 5 + wave; }
...
function update(dt) { if(gameState!=='playing') return; const move = (keys.ArrowLeft||keys.a?-1:0)+(keys.ArrowRight||keys.d?1:0)+touch.x; const verticalMove = (keys.w||keys.W?-1:0)+(keys.s||keys.S?1:0)+touch.y; const bounds = getCombatBounds(); const scale=getResponsiveScale(); player.x += Math.max(-1,Math.min(1,move))*300*dt; player.x=Math.max(bounds.min,Math.min(bounds.max,player.x)); player.y += Math.max(-1,Math.min(1,verticalMove))*240*dt; player.y=Math.max(90*scale,Math.min(height-65*scale,player.y)); player.cooldown=Math.max(0,player.cooldown-dt); player.timeSinceShot+=dt; const coolingRate=player.overheated>0?54.6:28.6; if(player.timeSinceShot>.25) player.overload=Math.max(0,player.overload-coolingRate*dt); if(player.overheated>0) player.overheated=Math.max(0,player.overheated-dt); if(keys[' ']||touch.fire) shoot(); if(waveDelay>0) waveDelay=Math.max(0,waveDelay-dt); spawnTimer-=dt; if(wave===5&&waveKills>=getWaveTarget()&&!boss) spawnBoss(); if(!boss&&waveDelay<=0&&waveKills<getWaveTarget()&&spawnTimer<=0){spawnEnemy(); spawnTimer=Math.max(.38,1.15-wave*.1);}
  ...
  for(let i=bullets.length-1;i>=0;i--){
    ...
    const bossHitboxWidth=aircraftImages.b2.width*1.5*scale*bossSizeMultiplier*.5;
    const bossHitboxHeight=aircraftImages.b2.height*1.5*scale*bossSizeMultiplier*.5;
