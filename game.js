...
function bossBallAttack() { const bounds=getCombatBounds(), speedMultiplier=boss.enraged?bossFuriousSpeed:1, ballCount=boss.enraged?(isMobileGame()?5:8):(isMobileGame()?4:8), verticalGap=isMobileGame()?145:105; fallingThreats=Array.from({length:ballCount},(_,index)=>({x:bounds.min+(bounds.max-bounds.min)*(index+1)/(ballCount+1),y:-60-index*verticalGap,speed:(145+index%3*18)*speedMultiplier,phase:index*.9,wobble:18})); playTone(170,.2,'square',.05,70); }
...
function getWaveTarget() { return isMobileGame() ? getMobileWaveTarget() : wave >= 4 ? wave + 2 : 5 + wave; }
